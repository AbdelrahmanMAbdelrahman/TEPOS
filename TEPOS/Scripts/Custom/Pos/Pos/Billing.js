...
var isCtrlSPressed = false; // Variable to track if Ctrl + S was pressed
let existingInvoiceIds = new Set();  // لتخزين معرفات الفواتير الحالية

$(document).ready(function () {
    globalThis.InvoiceThatFind = false;
    $('#btnOpenSearchPopup').click(function () {


        $('#invoiceSearchModal').modal('show');
    });

    $('#searchInvoiceButton').on('click', function () {
        ...
        var serviceUrl = '/Pos/GetSalesInvoicesForProduct';
        var jsonParam = { Name: productName };
        ...
        function onSuccess(data) {
            ...
            if (data.length === 0) {
                $resultsContainer.html("<p>No invoices found.</p>");
            } else {
                let tableHtml = `
        ...
        `;
                ...
                $.each(data, function (index, invoice) {
                    ...
                    tableHtml +=
                        ...
                        '<td>' + JsManager.ChangeDateFormat(invoice.Date, 0) + '</td>' +
        ...
        function onFailed(error) {
            ...
            $('#invoiceSearchResults').html("<p>An error occurred while fetching invoices.</p>");
        }
    ...
    $("#btnSaveInvoice").click(function () {
        ...
        $.each($('#dTableBilling tbody tr'), function (rowIdx, val) {
            let stockLevel = parseFloat($(val).find(".dtBillingStockQty").val()); // Assuming stock level is part of the row data
            let quantity = parseFloat($(val).find(".dtBillingQty").val()); // Assuming quantity is part of the row data
            var converFac = $(val).find(".dTablePosUomMasterId").val();
            // stockLevel = parseFloat(stockLevel == "" ? 0 : stockLevel) * parseFloat(converFac == "" ? 0 : converFac);
            quantity = parseFloat(quantity == "" ? 0 : quantity) * parseFloat(converFac == "" ? 0 : converFac);

            if ((stockLevel - quantity) < 0 && (globalThis.InvoiceThatFind == false)) {
                isStockValid = false;
                alert("Cannot add qty bigger than stock");
                return false; // Break out of the loop
            }

            if (parseFloat($(val).find(".dtBillingQty").val()) < 0 /*&& (globalThis.InvoiceThatFind == false)*/) {
                alert("Cannot add negative qty");
                isQtyValid = false;
                return false;
            }
        });
...
$(document).ready(function () {
    let existingInvoiceIds = new Set(); // To store the invoice IDs or other properties
    //let lengthOfInvoice = 0; // Variable to store the previous count of invoices

    // Periodically call the function to update the invoice list every 2 seconds
    setInterval(function () {
        ...
        function onSuccess(jsonData) {
            $("#cmbHoldInvoice").empty(); // Clear the dropdown list
            JsManager.PopulateCombo('#cmbHoldInvoice', jsonData, 'Hold Inv.', 0); // Populate the dropdown list with invoices

            let newInvoiceAdded = false; // Variable to track if a new invoice is added
            let lengthNewInvoices = jsonData.length; // Get the number of invoices (new data length)

            // Check if the number of invoices has increased
            if (lengthNewInvoices > globalThis.lengthOfInvoice) {
                // Show the success message if the number of invoices has increased
                Swal.fire({
                    title: 'New Invoice!',
                    html: 'A new invoice from Call Center',
                    icon: 'success',
                    confirmButtonText: 'OK'
                });
            }

            // Update the previous invoice count
            globalThis.lengthOfInvoice = lengthNewInvoices;

            // Check if there are new invoices based on their IDs
            jsonData.forEach(invoice => {
                if (!existingInvoiceIds.has(invoice.id)) {
                    existingInvoiceIds.add(invoice.id); // Add the invoice ID to the set of existing invoices
                    newInvoiceAdded = true; // Mark this as a new invoice
                }
            });
        }
    ...
    }, 50000); // Call the function every 2 seconds
});
...
var Manager = {
    ...
    GetDeliveryMan: function () {
        ...
        function onSuccess(response) {
            JsManager.PopulateCombo('#txtDeliveryMan', response, 'Select DeliveryMan', 0);
    ...
    LoadHoldInvoice: function () {
        ...
        function onSuccess(jsonData) {
            ...
            globalThis.lengthOfInvoice = jsonData.length;
        }
        ...
        if (invNumber == "") {
            ...
            function onSuccess(jsonData) {
                if (jsonData.toString().length > 0) {
                    ...
                    $('#dTableBilling tbody').empty(); // Clear existing rows														 
            ...
            if (Message.Prompt("Do you want to cancel this invoice?")) {
                ...
                function onSuccess(jsonData) {
                    if (jsonData > 0) {
                        ...
                        Message.Error("cannot cancel invoice created befor 14 days");
                    }
                    else {
    ...
    insertData: function (invoiceType) {
        ...
        if (dTableBilling.rows().count() > 0) {
            if (Message.Prompt("Do you want to save this invoice?")) {
                ...
                if (invoiceType == 1) {
                    var otherDiscount = $("#row-" + 0 + "-tenderAmount").val();
                    var cash = $("#row-" + 1 + "-tenderAmount").val();
                    var visa = $("#row-" + 2 + "-tenderAmount").val();
                    var bank = $("#row-" + 3 + "-tenderAmount").val();
                    var roundoff = $("#row-" + 4 + "-tenderAmount").val();
                    var payablamount = $("#row-" + 5 + "-tenderAmount").val();

                    if ((parseFloat(cash) + parseFloat(visa) + parseFloat(bank) + /*parseFloat(roundoff) +*/ parseFloat(otherDiscount)) != payablamount) {
                        Message.Error("payed amount should Same payabl amount for invoice ");
                        return;
                    }
                ...
                $.each($('#dTableBilling tbody tr'), function (rowIdx, val) {
                    ...
                    //2025
                    var QtyReturn = qty * converFac;
                    var prevQty = parseFloat($(val).find(".dtBillingPrevQty").val());
                    prevQty = parseFloat(prevQty == "" ? 0 : prevQty) * parseFloat(converFac == "" ? 0 : converFac);
                    if (!validateQuantityChange(QtyReturn, prevQty)) {
                        Message.Warning('Invalid return qty large that sales');
                        isValid = true;
                        return false;
                    }
                    /////
                    ...
                    debugger;
                    SDTO += parseFloat(objPosSalesInvoiceProduct.Sd);
                    VATTO += parseFloat(objPosSalesInvoiceProduct.Vat);

                });
                if (isValid == true) {
                    return false;
                }
                objInvoice.sdTotal = SDTO;
                objInvoice.VatTotal = VATTO;
                var otherDiscount = $("#row-" + 0 + "-tenderAmount").val();
                objInvoice.otherDiscount = otherDiscount;
                ...
                function onSuccess(jsonData) {
                    ...
                    else if (jsonData.rtrCode == "405") {
                        //Message.Warning("Failed To Create Bill. Invoice No <b style='color:red;'>" + jsonData.LastInv + "</b>");
                        Message.Error("User Code is required and Should be in Same Branch");
                    }
                    else if (jsonData.rtrCode == "10000") {
                        //Message.Warning("Failed To Create Bill. Invoice No <b style='color:red;'>" + jsonData.LastInv + "</b>");
                        Message.Error("You Must Open Shift");
                    }
                    else if (jsonData.rtrCode == "-00020") {
                        Message.Error("product or item returned several times");
                    }
                    else if (jsonData.rtrCode == "-00014") {
                        Message.Error("cannot return item when invoice created befor 14 days");
                    }
    ...
    applyScheme: function (data) {
        $.each($('#dTableBilling tbody tr'), function (rowIdx, val) {
            //$(val).find(".dtBillingDiscount").val(0.00); Adel (wrong Comment)
    ...
    BillingTotalCalculation: function (combiSchemeDiscount, otherDiscountObj) {
        ...
        dTableTender.rows().every(function (rowIdx, tableLoop, rowLoop) {
            ...
            if (tenderType == 'R') {
                ...
                Manager.changeInputBackgroundOfTender($("#row-" + 1 + "-tenderAmount"));
                Manager.changeInputBackgroundOfTender($("#row-" + rowIdx + "-tenderAmount"));
                cnd2 = true;
                Manager.dtTenderCalculation();
    ...
    AddBillingItem: function (prdCode) {
        ...
        function onSuccess(jsonData) {
            ...
            } else if (jsonData.BatchCount > 1 || jsonData.Stock <= 0) {
        ...
        if (isRef == "0") {
            dTableBilling = $('#dTableBilling').DataTable({
                ...
                columnDefs: [
                    ...
                    { className: "dt-head-right", targets: [0, 1] },
                    //{ visible: false, targets: [7] } // Hiding the PrevQty column (index 7)
                ],
                columns: [
                    {
                        name: '',
                    ...
                    {
                        data: 'PrevQty', // Add the "PrevQty" here
                        name: 'PrevQty',
                        orderable: false,
                        title: 'PrevQty', // Title for new column
                        width: 50, // Set width as needed
                        render: function (data, type, row, meta) {
                            return "<input type='number' id='row-" + meta.row + "-PrevQty' name='row-" + meta.row + "-PrevQty' value='" + data + "' class='form-control input-sm font-size-12px font-weight text-center dtBillingInputs dtBillingPrevQty minWidth50' style='display: none;' readonly='readonly'/>";
                        }
                    },
                    {
                        data: 'Sd',
    ...
    dtTenderCalculation: function () {
        ...
        dTableTender.rows().every(function (rowIdx, tableLoop, rowLoop) {
            ...
            if (sumOfColumn.includes(tenderType)) {
                if (tenderType == "CR") {
                    var paidableAmountChange = parseFloat($("#row-" + 5 + "-tenderAmount").val());
                    var RESULT = (paidableAmountChange - parseFloat($("#row-" + 2 + "-tenderAmount").val())
                        - parseFloat($("#row-" + 3 + "-tenderAmount").val()) - parseFloat($("#row-" + 0 + "-tenderAmount").val()));
                    if (RESULT < 0) {
                        RESULT = 0;
                    }
                    $("#row-" + 1 + "-tenderAmount").val(RESULT);
                }
                if (tenderType == "NA") {
                    var paidableAmountChange = parseFloat($("#row-" + 5 + "-tenderAmount").val());
                    var RESULT = (paidableAmountChange - parseFloat($("#row-" + 1 + "-tenderAmount").val())
                        - parseFloat($("#row-" + 3 + "-tenderAmount").val()) - parseFloat($("#row-" + 0 + "-tenderAmount").val()));
                    if (RESULT < 0) {
                        RESULT = 0;
                    }
                    $("#row-" + 2 + "-tenderAmount").val(RESULT);

                }
                if (tenderType == "NA") {
                    var paidableAmountChange = parseFloat($("#row-" + 5 + "-tenderAmount").val());
                    var RESULT = (paidableAmountChange - parseFloat($("#row-" + 1 + "-tenderAmount").val())
                        - parseFloat($("#row-" + 2 + "-tenderAmount").val()) - parseFloat($("#row-" + 0 + "-tenderAmount").val()));
                    if (RESULT < 0) {
                        RESULT = 0;
                    }
                    $("#row-" + 3 + "-tenderAmount").val(RESULT);
                }
...
//2025
function validateQuantityChange(QtyReturn, prevQty) {
    if ((QtyReturn < -prevQty) && (globalThis.InvoiceThatFind == true)) {
        return false;
    }
    return true;
}
...
$(document).on("keyup", ".dTableProductCode, #txtItemOrBarcode", function (itemKey) {
    ...
    if (elCheckBox.checked && elInput.value.length > 0) {
        ...
        var jsonParam = { PosBranchId: -1, Parcode: elInput.value, Name: elInput.value, ScientificName: '' };
        JsManager.SendJsonAsyncON(serviceUrl, jsonParam, onSuccess, onFailed);
...
$(document).on("keyup", ".dTableProductCode, #txtScientificName", function (itemKey) {
    if (itemKey.key == "F2") {
        ...
        if (elCheckBox.checked && elInput.value.length > 0) {
            ...
            var serviceUrl = '/Pos/GetProductInfoCallCenter/';
            var jsonParam = { PosBranchId: -1, Parcode: '', Name: '', ScientificName: elInput.value };
            JsManager.SendJsonAsyncON(serviceUrl, jsonParam, onSuccess, onFailed);
...
$(document).on("keyup", ".dTableProductCode, #txtProductName", function (itemKey) {
    if (itemKey.key == "F2") {
        ...
        if (elCheckBox.checked && elInput.value.length > 0) {
            ...
            //var PosBranchId = $("#PosBranchId").val();
            var serviceUrl = '/Pos/GetProductInfoCallCenter/';
            var jsonParam = { PosBranchId: -1, Parcode: '', Name: elInput.value, ScientificName: '' };
            JsManager.SendJsonAsyncON(serviceUrl, jsonParam, onSuccess, onFailed);
...
$(document).on('keydown', function (e) {
    ...
    if (e.key == "F7") {
        ...
        if (!isCtrlSPressed) { // Check if Ctrl + S has been pressed before
            isCtrlSPressed = true; // Mark it as pressed
            e.preventDefault(); // Prevent the default browser behavior

            // Execute the save functionality on the first press
            if ($("#btnSaveInvoiceStockTrns").length > 0) {
                $("#btnSaveInvoiceStockTrns").trigger("click");
            } else {
                $("#btnSaveInvoice").trigger("click");
            ...
            e.preventDefault(); // Prevent further execution if pressed again
        }
    } else if (e.ctrlKey && e.keyCode == 67) {
        //cancel invoice 
        e.preventDefault();
